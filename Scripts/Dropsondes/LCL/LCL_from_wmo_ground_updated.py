...
bco_file = sorted(glob.glob('BCO_Sounding*.nc'))

bco_sp = xr.open_dataset(bco_file[0])

lcl_ht = [None] *len(allfiles)

for i in range(len(allfiles)) :
    lcl_ht[i] = bco_sp['altitude'].where((bco_sp['pressure']<lcl_p[i]+1) & (bco_sp['pressure']>lcl_p[i]-1)).mean().values
    print('LCL Height for ' + str(time_id[i]) 
          + ' UTC is ' + str(lcl_ht[i]) + ' m.')


# In[9]:


ds = xr.Dataset({#'temp'           : (['time','pres'],temp),
                 'lcl_p'               : (['time'],lcl_p),
                 'lcl_t'            : (['time'],lcl_t),
                 'lcl_ht'           : (['time'],lcl_ht),
                 'lat'            : (['time'],lat),
...
plt.plot(ds['time'],#.where(ds['lon'] < sel_lon,drop=True).values,
        ...
        marker='o')
plt.gca().axhline(ds['lcl_p'].mean(),c='r',linewidth='3',label='Mean',#.where(ds['lon'] < sel_lon,drop=True).values,
        marker='o')
...
plt.plot(ds['time'].where(ds['lon'] < sel_lon,drop=True).values,
         ds['lcl_p'].where(ds['lon'] < sel_lon,drop=True).values,
        marker='o')
plt.gca().axhline(ds['lcl_p'].where(ds['lon'] < sel_lon,drop=True).mean(),c='r',linewidth='3',label='Mean',
        marker='o')
myFmt = mdates.DateFormatter('%H:%M')
plt.gca().xaxis.set_major_formatter(myFmt)
plt.legend(frameon=False)
...
plt.xlabel('Time / UTC')
plt.ylabel('Pressure / hPa')
plt.savefig('lcl_p_west_of_'+str(sel_lon)+'W_vs_time.png')
...
plt.plot(ds['time'],#.where(ds['lon'] > sel_lon,drop=True).values,
         ds['lcl_ht'],#.where(ds['lon'] > sel_lon,drop=True).values,
        marker='o')
plt.gca().axhline(ds['lcl_ht'].mean(),linewidth=3,c='r',label='Mean')
myFmt = mdates.DateFormatter('%H:%M')
plt.gca().xaxis.set_major_formatter(myFmt)
plt.legend(frameon=False)
plt.gca().spines['right'].set_visible(False)
...
plt.savefig('lcl_ht_all_vs_time.png')
...
plt.plot(ds['time'].where(ds['lon'] > sel_lon,drop=True).values,
         ds['lcl_ht'].where(ds['lon'] > sel_lon,drop=True).values,
        marker='o')
plt.gca().axhline(ds['lcl_ht'].where(ds['lon'] > sel_lon,drop=True).mean(),linewidth=3,c='r',label='Mean')
myFmt = mdates.DateFormatter('%H:%M')
plt.gca().xaxis.set_major_formatter(myFmt)
plt.legend(frameon=False)
plt.gca().spines['right'].set_visible(False)
...
plt.xlabel('Time / UTC')
plt.ylabel('Altitude / m')
plt.savefig('lcl_ht_west_of_'+str(sel_lon)+'W_vs_time.png')
